...
$query="SELECT Nome,  Prezzo, datainiziale, datafinale
            FROM SpesaLungoTermine
            ORDER BY Nome";
...
for($i = (($pagina)*20); $i<(($pagina+1)*20) && $i < $numerorisultati; $i++ ){
    $nom[] = $resulset[$i][0];
    $prezz[] = $resulset[$i][1];
    $datainiziale[] = $resulset[$i][2];
    $datafinale[] = $resulset[$i][3];
}
mysqli_free_result($res);
print(json_encode(array("nome" => json_encode($nom),  "prezzo" => json_encode($prezz),
        "datainizio"=>json_encode($datainiziale) ,"datafine"=> json_encode($datafinale), "numeropagine" => $numeropagine )));
