...
$query="SELECT Nome, fornitore, Prezzo
            FROM ingredienti
            ORDER BY Nome";
...
for($i = (($pagina)*20); $i<(($pagina+1)*20) && $i < $numerorisultati; $i++ ){
    $nom[] = $resulset[$i][0];
    $fornitore[] = $resulset[$i][1];
    $prezz[] = $resulset[$i][2];
}
print(json_encode(array("nome" => json_encode($nom), "fornitore" => json_encode($fornitore) , "prezzo" => json_encode($prezz), "numeropagine" => $numeropagine )));
